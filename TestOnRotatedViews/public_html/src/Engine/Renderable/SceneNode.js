...
SceneNode.prototype.draw = function (aCamera, deg, parentMat) {
    var i;
    ...
    for (i = 0; i < this.mSet.length; i++) {
        this.mSet[i].draw(aCamera, deg, xfMat); // pass to each renderable
    }
    
    // now draw the children
    for (i = 0; i < this.mChildren.length; i++) {
        this.mChildren[i].draw(aCamera, deg, xfMat); // pass to each renderable
    }
